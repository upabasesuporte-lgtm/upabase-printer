  ...
  return (
    <div className="relative overflow-hidden rounded-2xl p-5 cursor-default" style={cardStyle}>
      ...
      <div className="relative z-10">
        ...
        <div className="text-xl sm:text-2xl font-black tabular-nums break-all"
          style={{ background:`linear-gradient(135deg,${gFrom},${gTo})`, WebkitBackgroundClip:"text", display:"block",WebkitTextFillColor:"transparent", backgroundClip:"text" }}>
          {value}
        </div>
...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    const allAp = apRes.data ?? [];
    console.log("📊 DEBUG Despesas:", { period, fromDate, toDate, totalRecords: allAp.length });
    if (allAp.length > 0) {
      console.log("Primeira despesa raw:", allAp[0]);
    }
    const filteredAp = allAp.filter((b: any) => {
      ...
      const matches = dateRefLocal >= fromDate && dateRefLocal <= toDate;
      if (allAp.length <= 5) {
        console.log(`  Despesa: ${dateRef} → ${dateRefLocal} (range: ${fromDate}-${toDate}) → ${matches ? "✓" : "✗"}`);
      }
      return matches;
    });
    console.log(`Filtradas: ${filteredAp.length} de ${allAp.length}`);
        ...
        <div className="relative flex items-center justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[11px] font-semibold uppercase tracking-widest" style={{ color: isLight ? "#3B82F6" : "#a1a1aa" }}>Ao vivo</span>
            </div>
            <h1 className="text-2xl font-black" style={{ color: isLight ? "#3B82F6" : "var(--gradient-text, #fff)" }}>
              {greeting}!
            </h1>
        ...
        <div className="rounded-2xl p-5" style={{ background: card.bg, border: card.border, boxShadow: card.shadow }}>
          ...
          <div className="space-y-3">
            {topSellers.map((seller, idx) => {
              const maxTotal = topSellers[0]?.total || 1;
              ...
              return (
                <div key={seller.name}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <p className="text-sm font-black flex-shrink-0" style={{ color: "#8b5cf6" }}>
                      {fmt(seller.total)}
                    </p>
                  </div>
                  <div className="h-2 rounded-full overflow-hidden" style={{ background: isLight ? "#f3f4f6" : "#27272a" }}>
                    <div className="h-full rounded-full transition-all" style={{ width: `${percentage}%`, background: "linear-gradient(90deg,#8b5cf6,#a78bfa)" }} />
                  </div>
                </div>
              );
            })}
